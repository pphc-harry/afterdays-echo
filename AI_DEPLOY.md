...
這個版本已經加入 `/api/chat`，可在 Vercel 上安全呼叫 AI API。

## 部署到 Vercel

1. 打開 Vercel 匯入頁：
   https://vercel.com/new/clone?repository-url=https://github.com/pphc-harry/afterdays-echo&env=GEMINI_API_KEY&env=GEMINI_MODEL
...
- 前端會先呼叫 `./api/chat`。
- 後端會優先使用 `GEMINI_API_KEY`，沒有 Gemini key 才使用 `OPENAI_API_KEY`。
